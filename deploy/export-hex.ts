...
function main() {
  ...
  writeFileSync(`./build/amm-minter.build.json`, `{ "hex":"${ammMinterCodeB64}", "commitHash":"${getCommitHash}" }`);
  const ammWalletCode: string = compileFuncToB64(["contracts/amm-wallet.fc"]);
  writeFileSync(`./build/amm-wallet.build.json`, `{ "hex":"${ammWalletCode}", "commitHash":"${getCommitHash}" }`);
}
